...
class ResNetCausalOnlyHead(nn.Module):
    """
    """
    ...
    def __init__(
        ...
        Args:
            ...
            act_func (string): activation function to use. 'softmax': applies
                softmax on the output. 'sigmoid': applies sigmoid on the output.
        """
        super(ResNetCausalOnlyHead, self).__init__()
        assert (
            len({len(pool_size), len(dim_in)}) == 1
        ), "pathway dimensions are not consistent."
        self.num_pathways = len(pool_size)
        ...
        logger.info('Use Causal Only head')
...
class ResNetRecognitionMTLHead(nn.Module):
    """
    ResNe(X)t 3D head.
    This layer performs a fully-connected projection during training, when the
    input size is 1x1x1. It performs a convolutional projection during testing
    ...
    def __init__(
        ...
        Args:
            ...
            act_func (string): activation function to use. 'softmax': applies
                softmax on the output. 'sigmoid': applies sigmoid on the output.
        """
        super(ResNetRecognitionMTLHead, self).__init__()
        assert (
            len({len(pool_size), len(dim_in)}) == 1
        ), "pathway dimensions are not consistent."
        self.num_pathways = len(pool_size)
        ...
        # Perform FC in a fully convolutional manner. The FC layer will be
        # initialized with a different std comparing to convolutional layers.
        self.projection_predict = nn.Linear(sum(dim_in), num_classes, bias=True)
        self.projection_recog = nn.Linear(sum(dim_in), num_classes, bias=True)
        #self.num_classes = num_classes
        ...
        logger.info('Use Recognition Multitask head')
    ...
    def forward(self, inputs):
        ...
        # if isinstance(self.num_classes, (list, tuple)):
        x_predict = self.projection_predict(x)
        x_recog = self.projection_recog(x)

        # Performs fully convlutional inference.
        if not self.training:
            x_predict = self.act(x_predict)
            x_predict = x_predict.mean([1, 2, 3])

            x_recog = self.act(x_recog)
            x_recog = x_recog.mean([1, 2, 3])
        x_predict = x_predict.view(x_predict.shape[0], -1)
        x_recog = x_recog.view(x_recog.shape[0], -1)
        return (x_predict, x_recog)
