...
class Recognition_MTLTrainMeter(object):
    """
    Measure training stats.
    """
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        # log observed frame's action recognition performance
        #self.recog_num_top1_mis = 0
        #self.recog_num_top5_mis = 0
    ...
    #def update_stats(self, top1_err, top5_err, loss, lr, mb_size, recog_top1_err=None, recog_top5_err=None):
    def update_stats(self, recog_top1_err, recog_top5_err, loss, mb_size):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
        ...
        # Current minibatch stats
        self.mb_top1_err.add_value(recog_top1_err)
        self.mb_top5_err.add_value(recog_top5_err)
        # Aggregate stats
        self.num_top1_mis += recog_top1_err * mb_size
        self.num_top5_mis += recog_top5_err * mb_size

        #self.recog_num_top1_mis += top1_err[1] * mb_size
        #self.recog_num_top5_mis += top5_err[1] * mb_size
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            #"time_diff": self.iter_timer.seconds(),
            #"eta": eta,
            "observed_action_loss": self.loss.get_win_median(),
            #"lr": self.lr,
            #"gpu_mem": "{:.2f} GB".format(misc.gpu_mem_usage()),
        }
        stats["observed_action_top1_err"] = self.mb_top1_err.get_win_median()
        stats["observed_action_top5_err"] = self.mb_top5_err.get_win_median()
        logging.log_json_stats(stats)
    ...
    def log_epoch_stats(self, cur_epoch, writer=None):
        ...
        stats["observed_action_top1_err"] = top1_err
        stats["observed_action_top5_err"] = top5_err
        stats["observed_action_loss"] = avg_loss

        # log observed frame's action recognition acc
        #recog_top1_err = self.recog_num_top1_mis / self.num_samples
        #recog_top5_err = self.recog_num_top5_mis / self.num_samples
        #stats["observed_top1_err"] = recog_top1_err
        #stats["observed_top5_err"] = recog_top5_err

        if writer is not None:
            #if not self.multitask:
            writer.add_scalars(
                {
                    "Train/Observed_Action_Avg_loss": avg_loss,
                    "Train/Epoch_Observed_Action_Top1_err": top1_err,
                    "Train/Epoch_Observed_Action_Top5_err": top5_err,
                    #"Train/Epoch_lr": self.lr,
                },
                global_step = cur_epoch,
            )
...
class Recognition_MTLValMeter(object):
    """
    Measures validation stats.
    """
    ...
    def update_stats(self, recog_top1_err, recog_top5_err, mb_size):
        """
        Update the current stats.
        Args:
            recog_top1_err (float): observed action top1 error rate.
            recog_top5_err (float): observed action top5 error rate.
            mb_size (int): mini batch size.
        """
        self.num_samples += mb_size
    ...
    def log_epoch_stats(self, cur_epoch, writer=None):
        ...
        if writer is not None:
            writer.add_scalars(
                {
                    "Val/Epoch_Observed_Action_Top1_err": top1_err,
                    "Val/Epoch_Observed_Action_Top5_err": top5_err,
                },
                global_step = cur_epoch,
            )
...
class Recognition_MTLTestMeter(object):
    """
    Perform the multi-view ensemble for testing: each video with an unique index
    will be sampled with multiple clips, and the predictions of the clips will
    be aggregated to produce the final prediction for the video.
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        assert len({len(ks), len(topks)}) == 1
        for k, topk in zip(ks, topks):
            stats["observed_action_top{}_acc".format(k)] = "{:.{prec}f}".format(
                topk, prec=2
            )
        logging.log_json_stats(stats)
